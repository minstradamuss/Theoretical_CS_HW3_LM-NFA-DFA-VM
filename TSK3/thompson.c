...
static void addthread(struct ThreadList *l, struct Thread t)
{
  ...
  // printf("addthread(): t.opcode=%d, t.gen=%d, l->n=%d\n",
  //        t.pc->opcode,
  //        t.pc->gen,
  //        l->n);
  //printf(__FILE__ ":%d\n", __LINE__);
...
static void test_quest(void)
{
  ...
  struct Prog *prog = compile(re);
  char *sub[MAXSUB];
  assert(1 == is_match_thompson(prog, "b", sub));
  assert(2 == is_match_thompson(prog, "ba", sub));
  assert(3 == is_match_thompson(prog, "baa", sub));
  assert(3 == is_match_thompson(prog, "baaaaa", sub));
  assert(0 == is_match_thompson(prog, "aaaaab", sub));
  free(prog);
}
